...
function query() {
    return storageService.query(DB_KEY)
        .then(locs => {
            ...
            } else if (gSortBy.creation_time !== undefined) {
                locs.sort((p1, p2) => (p1.createdAt - p2.createdAt) * gSortBy.creation_time)
            }
    ...
    return storageService.query(DB_KEY)
        .then(locs => {
            const locCountByLastUpdatedMap = locs.reduce((map, loc) => {
                ...
                else if (_hoursSinceEpoch(loc.updatedAt) <= 24) map.today++
                else map.past++
                return map
...
function _hoursSinceEpoch(updatedAt) {
